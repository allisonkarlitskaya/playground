  ...
  push:
    ...
    steps:
      ...
      - name: Ensure no workflow changes
        run: git diff --exit-code '${{ github.sha }}..${{ github.event.workflow_run.head_sha }}' -- .github

      - name: Generate diff between base and PR package.json
        run: git diff '${{ github.sha }}..${{ github.event.workflow_run.head_sha }}' -- package.json > package.json.diff
      ...
      - name: Verify package.json matches PR head commit
        run: |
          set -ux
          git cat-file blob '${{ github.event.workflow_run.head_sha }}:package.json' > package.json
      ...
      - name: Stage git commit
        run: |
          ...
          git --work-tree node_modules commit --quiet -F - < artifact/npm-install.log
